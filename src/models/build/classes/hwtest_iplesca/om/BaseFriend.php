...
abstract class BaseFriend extends BaseObject implements Persistent
{
    ...
    /**
     * The value for the friendid field.
     * @var        int
     */
    protected $friendid;
    ...
    /**
     * Get the [friendid] column value.
     * users table id of friend
     * @return int
     */
    ...
    public function setUserid($v)
    {
        ...
        if ($this->userid !== $v) {
            $this->userid = $v;
            $this->modifiedColumns[] = FriendPeer::USERID;
        }
    ...
    /**
     * Set the value of [friendid] column.
     * users table id of friend
     * @param int $v new value
     * @return Friend The current object (for fluent API support)
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->friendid = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
            $this->resetModified();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(FriendPeer::USERID)) {
            $modifiedColumns[':p' . $index++]  = '`userId`';
        }
        if ($this->isColumnModified(FriendPeer::FRIENDID)) {
            $modifiedColumns[':p' . $index++]  = '`friendId`';
        }

        $sql = sprintf(
            'INSERT INTO `friends` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`userId`':
                        ...
                        break;
                    case '`friendId`':
                        $stmt->bindValue($identifier, $this->friendid, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(FriendPeer::USERID)) $criteria->add(FriendPeer::USERID, $this->userid);
